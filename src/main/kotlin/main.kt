fun main() {
    println("Day 1, first part: " + ReportRepair().solveFirstPart())
    println("Day 1, second part: " + ReportRepair().solveSecondPart())

    println("Day 2, first part: " + PasswordPhilosophy().solveFirstPart())
    println("Day 2, second part: " + PasswordPhilosophy().solveSecondPart())

    println("Day 3, first part: " + TobogganTrajectory().solveFirstPart())
    println("Day 3, second part: " + TobogganTrajectory().solveSecondPart())

    println("Day 4, first part: " + PassportProcessing().solveFirstPart())
    println("Day 4, second part: " + PassportProcessing().solveSecondPart())

    println("Day 5, first part: " + BinaryBoarding().solveFirstPart())
    println("Day 5, second part: " + BinaryBoarding().solveSecondPart())

    println("Day 6, first part: " + CustomCustoms().solveFirstPart())
    println("Day 6, second part: " + CustomCustoms().solveSecondPart())
}

